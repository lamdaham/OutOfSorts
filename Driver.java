  ...
  public static void main(String[]args){
    if(args.length < 2){
      ...
      if(args.length < 3 || args[2].equals("random")){
        ...
        int eq = random.nextInt();
	      for(int i = 0; i<randData.length; i++) {
          randData[i] = eq;
        }
      }else if(args[2].equals("sorted")){
        randData[0] = random.nextInt();
	      for(int i = 1; i<randData.length; i++) {
          randData[i] = randData[i-1] + random.nextInt();
        }
      }else if(args[2].equals("reversed")){
	      randData[randData.length-1] = random.nextInt();
        for(int i = randData.length-2; i>=0; i--) {
          randData[i] = randData[i+1] + random.nextInt();
        }
      }
